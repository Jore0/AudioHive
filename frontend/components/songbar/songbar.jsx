...
class SongBar extends React.Component {
  ...
  updateTime() {
    this.audio.current.onplaying = () => {
      this.currentTime = setInterval(() => {
        ...
        this.props.updateCurrentSongTime(
          this.state.currentTime / this.state.duration
        );
        this.props.currentSongTimeMins(this.state.currentTime);
      }, 100);
    };
    ...
    this.audio.current.onended = () => {
      clearInterval(0), this.setState({ currentTime: 0 });
    };
  }
  changePlace(e) {
  ...
  render() {
    const status = !this.state.currentlyPlaying ? (
      <i className="fas fa-play"></i>
    ) : (
      <i className="fas fa-pause"></i>
    );
    let currentSong = this.state.currentSong;
    let currentTime = this.state.readyToPlay
      ? this.secondsToMinutes(this.state.currentTime)
      : 0;
      ...
      <>
        <div className="songBar-container">
          <section className="songBar-container-parts">
            ...
            <button
              className="songBar-control play pause"
            ...
            <div className="seek-bar-volume-container">
              <button className="songBar-control" onClick={this.mute}>
                <i
                  className={
                    !this.state.mute ? "fas fa-volume-up" : "fas fa-volume-mute"
                  }
                ></i>
              </button>
              <input
